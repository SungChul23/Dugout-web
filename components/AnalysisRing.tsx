  ...
  return (
    <div className="relative w-full h-[400px] md:h-[600px] flex items-center justify-center overflow-visible perspective-container my-4 md:my-12">
      ...
      <div 
        ...
        style={{ width: `${dimensions.centerWidth}px`, height: `${dimensions.centerHeight}px` }}
      >
        {/* Glass Backdrop */}
      ...
      >
        {TEAMS.map((team, index) => {
          ...
          return (
            <div
              key={team.id}
              className="absolute top-1/2 left-1/2 transform-style-3d backface-visible transition-all duration-500"
              style={{
              ...
              <div className="w-full h-full relative group transition-transform duration-500 hover:scale-105">
                <div 
                  className={`
                    ...
                    flex flex-col p-4 md:p-6 transition-all duration-300
                ...
                >
                  ...
                  <div className="flex justify-between items-start mb-4 md:mb-6">
                    <span className="text-xl md:text-2xl font-black text-white/10 font-mono tracking-tighter">#{team.rank}</span>
                    <div className={`w-2 h-2 md:w-2.5 md:h-2.5 rounded-full shadow-[0_0_8px_currentColor] ${team.rank <= 3 ? 'text-blue-500 bg-blue-500' : team.rank <= 6 ? 'text-red-500 bg-red-500' : 'text-slate-600 bg-slate-600'}`}></div>
                  </div>

                  {/* Team Code */}
                  <div className="flex-1 flex flex-col items-center justify-center">
                    <h3 className="text-2xl md:text-3xl font-black italic tracking-tighter text-white font-sans uppercase">
                      {team.code}
                    </h3>
                    <p className="text-[8px] md:text-[10px] text-slate-500 font-mono mt-1 tracking-widest">{team.name}</p>
                  </div>

                  {/* Stats Section */}
                  <div className="mt-auto pt-4 md:pt-5 border-t border-white/5 space-y-2 md:space-y-3">
                    <div className="flex justify-between items-end">
                      <span className="text-[8px] md:text-[9px] text-slate-500 font-bold uppercase tracking-wider">Win Rate</span>
                      <span className="text-base md:text-lg font-bold text-brand-accent font-mono leading-none">{team.winRate}</span>
                    </div>
      ...
      {/* Decorative Floor */}
      <div 
        className="absolute bottom-[-100px] w-[600px] md:w-[1000px] h-[600px] md:h-[1000px] rounded-full opacity-5 transform rotate-x-90 pointer-events-none"
        style={{ 
